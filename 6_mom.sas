...
if startdate<='31jan2017'd;
run;

data five.fund_benchmark_weight4_3y;
set five.fund_benchmark_weight41;
if startdate<='31dec2014'd;
...
/*
proc corr data=five.fund_benchmark_weight4 nomiss outp=CorrOutp;
   var smb hml mom;
   by windcode;
run;
...
model excess_return = sys_risk smb hml mom;
by windcode;
quit;
...
model benchmark_adjusted_return = sys_risk smb hml mom;
by windcode;
quit;
data five.a1 five.a2;
...
select a.windcode, a.fundname, a.t_alpha as alpha_bench, b.t_alpha as alpha_rf
...
/*total sample TM risk-free-rate*/
  ...
  model excess_return = sys_risk sys_square smb hml mom;
  by windcode;
  quit;
  data five.a1 five.a2;
  ...
  model benchmark_adjusted_return = sys_risk sys_square smb hml mom;
  by windcode;
  quit;
  data five.a1 five.a2;
  ...
  select a.windcode, a.fundname, a.t_gamma as gamma_bench, b.t_gamma as gamma_rf
...
data five.Rf_bench_compare1;
set five.Rf_bench_compare1;
if alpha_bench>1.64 then sig_alpha_bench='正显著';
else if alpha_bench<-1.64 then sig_alpha_bench='负显著';
else sig_alpha_bench='不显著';

if alpha_rf>1.64 then sig_alpha_rf='正显著';
else if alpha_rf<-1.64 then sig_alpha_rf='负显著';
else sig_alpha_rf='不显著';
run;

data five.Rf_bench_compare1_3y;
set five.Rf_bench_compare1_3y;
if alpha_bench>1.64 then sig_alpha_bench='P';
else if alpha_bench<-1.64 then sig_alpha_bench='N';
else sig_alpha_bench='O';

if alpha_rf>1.64 then sig_alpha_rf='P';
else if alpha_rf<-1.64 then sig_alpha_rf='N';
else sig_alpha_rf='O';
run;
proc freq data=five.Rf_bench_compare1_3y noprint;
...
data five.Rf_bench_compare1_3y_TM;
set five.Rf_bench_compare1_3y_TM;
if gamma_bench>1.64 then sig_gamma_bench='P';
else if gamma_bench<-1.64 then sig_gamma_bench='N';
else sig_gamma_bench='O';

if gamma_rf>1.64 then sig_gamma_rf='P';
else if gamma_rf<-1.64 then sig_gamma_rf='N';
else sig_gamma_rf='O';
run;
proc freq data=five.Rf_bench_compare1_3y_TM noprint;
...
var alpha_year beta smb hml mom rsq;
class group;
...
var gamma alpha_year beta smb hml mom rsq;
class group;
...
data interestingly;
retain sys_risk smb hml mom benchmark_adjusted_return pred resid;
set interestingly;
keep sys_risk smb hml mom benchmark_adjusted_return pred resid;
run;quit;
...
equation=benchmark_adjusted_return=pred_a+resid,random=5673);
...
data interestingly;
retain sys_risk sys_square smb hml mom benchmark_adjusted_return pred resid;
set interestingly;
keep sys_risk sys_square smb hml mom benchmark_adjusted_return pred resid;
run;quit;
...
equation=benchmark_adjusted_return=pred_g+resid,random=5673);
...
%let m=20180105s;*fill in export date;

PROC EXPORT DATA=five.Reg_fbw4_bench2
FILE="&address.mutual_tables_bench_alpha&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_BR全样本";
RUN;
...
FILE="&address.mutual_tables_bench_gamma&m"
DBMS=xlsx REPLACE;
SHEET="择时能力_BR全样本";
RUN;
...
FILE="&address.mutual_tables_rf_alpha&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_ER全样本";
RUN;
...
FILE="&address.mutual_tables_rf_gamma&m"
DBMS=xlsx REPLACE;
SHEET="择时能力_ER全样本";
RUN;
...
FILE="&address.mutual_tables_bootstrap_pvalues&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_BR5y";
RUN;
...
FILE="&address.mutual_tables_bench_shizu&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_BR3y";
RUN;
...
FILE="&address.mutual_tables_rf_shizu&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_ER3y";
RUN;
...
FILE="&address.mutual_tables_compare_alpha&m"
DBMS=xlsx REPLACE;
SHEET="选股能力_全样本";
RUN;
...
FILE="&address.mutual_tables_compare_gamma&m"
DBMS=xlsx REPLACE;
SHEET="择时能力_全样本";
RUN;
...
FILE="&address.mutual_tables_summary&m"
DBMS=xlsx REPLACE;
SHEET="选股能力";
RUN;
...
data temp;
set five.mutualbasic;
where _COL8 in ('普通股票型基金','偏股混合型基金', '灵活配置型基金');
run;
